...
object MathTools {
    ...
    private fun registerFft() {
        ToolRegistry.register(ToolDefinition(
            ...
            execute       = { args ->
                ...
                if (rawData.size < 2) return@ToolDefinition """{"error":"data must have at least 2 elements"}"""
                ...
                """{"n_samples":${rawData.size},"n_padded":$n,"sample_rate_hz":$sampleRate,"top_frequencies":[$binsJson]}"""
            }
        ))
    }
    ...
    private fun registerFitCurve() {
        ToolRegistry.register(ToolDefinition(
            ...
            execute       = { args ->
                val x      = parseDoubleArray(args, "x") ?: return@ToolDefinition """{"error":"'x' required"}"""
                val y      = parseDoubleArray(args, "y") ?: return@ToolDefinition """{"error":"'y' required"}"""
                val degree = extractInt(args, "degree") ?: 1

                if (x.size != y.size) return@ToolDefinition """{"error":"x and y must have the same length"}"""
                if (x.size < degree + 1) return@ToolDefinition """{"error":"need at least degree+1 data points"}"""
                if (degree < 1 || degree > 5) return@ToolDefinition """{"error":"degree must be 1–5"}"""
                ...
                """{"degree":$degree,"type":"$degreeLabel","coefficients":{$coeffJson},"r_squared":${"%.4f".format(r2)},"equation":"${polyEquation(coeffs)}"}"""
            }
        ))
    }
        ...
        ToolRegistry.register(ToolDefinition(
            ...
            execute       = { args ->
                val op = Regex("\"operation\"\\s*:\\s*\"([^\"]+)\"").find(args)?.groupValues?.get(1)
                    ?: return@ToolDefinition """{"error":"'operation' required: distance, bearing, or polygon_area"}"""

                when (op) {
                    "distance" -> {
                        val lat1 = extractDouble(args, "lat1") ?: return@ToolDefinition """{"error":"lat1 required"}"""
                        val lon1 = extractDouble(args, "lon1") ?: return@ToolDefinition """{"error":"lon1 required"}"""
                        val lat2 = extractDouble(args, "lat2") ?: return@ToolDefinition """{"error":"lat2 required"}"""
                        val lon2 = extractDouble(args, "lon2") ?: return@ToolDefinition """{"error":"lon2 required"}"""
                        val km   = haversineKm(lat1, lon1, lat2, lon2)
                        """{"distance_km":${"%.3f".format(km)},"distance_miles":${"%.3f".format(km * 0.621371)},"distance_m":${"%.1f".format(km * 1000)}}"""
                    }
                    "bearing" -> {
                        val lat1 = extractDouble(args, "lat1") ?: return@ToolDefinition """{"error":"lat1 required"}"""
                        val lon1 = extractDouble(args, "lon1") ?: return@ToolDefinition """{"error":"lon1 required"}"""
                        ...
                        """{"bearing_deg":${"%.1f".format(bearing)},"compass":"${compassPoint(bearing)}"}"""
                    }
                    "polygon_area" -> {
                        val points = parseLatLonArray(args)
                            ?: return@ToolDefinition """{"error":"'points' must be an array of {\"lat\":...,\"lon\":...}"}"""
                        if (points.size < 3) return@ToolDefinition """{"error":"polygon needs at least 3 points"}"""
                        val areaM2 = polygonAreaM2(points)
                        """{"area_m2":${"%.1f".format(areaM2)},"area_km2":${"%.4f".format(areaM2 / 1e6)},"area_acres":${"%.2f".format(areaM2 / 4046.856)},"area_hectares":${"%.4f".format(areaM2 / 10000)}}"""
                    }
                    else -> """{"error":"Unknown operation '$op'. Use: distance, bearing, polygon_area"}"""
                }
            }
        ))
    }
