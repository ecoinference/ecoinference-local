...
- **`use tool` was broken for anything location- or time-dependent — fixed (`68284d6`).** The
...
- **Generated Python now collapsed behind a "Show Python" toggle (`e9f3c4f`, Android).** The
...
- **Firestore + Storage security rules — written, committed and deployed (`731264b`).** They
  ...
  **Deploying revealed that neither backend existed.** The CLI logged
  ...
  `--only storage`.
- **Third-party licensing — all three resolved (`49f1935`, `c932a58`, `9d1c7fd`).** None was
...
- **Android: fixed "Provided less images than expected" crash on the turn after any image
  ...
  change — confirmed unaffected by both code review and a live test. `05c20ca`.
...
- Delete confirmation dialog on Models screen, both platforms (`e191d78`).
- Storage pre-flight check before starting a model download, both platforms — throws a clear
  "needs ~X MB, only Y MB free" error instead of failing mid-download (`7c56a63`).
- Budget-exhausted forced-final turn in `AgentLoop`, both platforms — when the tool-call
  ...
  final no-tools turn, instead of just breaking the loop (`7c56a63`).
- Download speed + ETA shown in Models screen UI, both platforms (`74736ea`).
- Tool-result security hardening: `wrapUntrusted()` (nonce-delimited markers around tool
  ...
  and `truncateToolResult()` (6000-char cap), both platforms (`74736ea`, plus earlier
  `run_python` findings).
- First-ever test coverage on either platform: iOS `AIiOSTests` (standalone target, no host
  ...
  `wrapUntrusted`, `truncateToolResult` (`74736ea`).
- iPad blank-screen bug fixed (`UITextEffectsWindow` system overlay was getting the same
  opaque-background treatment as app windows) (`10937e1`).
- iOS model-download timeout + tool-call parse fallback fixes (`dd10016`).
- Android `minSdk` bumped 26 → 30 — `.litertlm` models require API 30+; a Galaxy S9 (Android
  10/API 29) was confirmed to hard-fail loading the native `libLiteRtLm.so` (`d68d2af`).
...
### Recently completed
- **Offline use after first login — fixed, NEEDS VERIFYING ON WINDOWS (`f21c03a`).** Reported
...
- **Qwen3-VL-4B-Instruct added to the model catalog (Windows ARM64, `e49bb59`)** — second
